...
class OcrToTableTool:
    ...
    def execute(self):
        ...
        return self.generate_orderidlist()

    def threshold_image(self):
        return cv2.threshold(self.grey_image, 0, 255)[1]
    ...
    def convert_contours_to_bounding_boxes(self):
        ...
        for contour in self.contours:
            ...
            if(w*h>1000 and w<250 and h<100):
                self.bounding_boxes.append((x, y, w, h))
                self.image_with_all_bounding_boxes = cv2.rectangle(self.image_with_all_bounding_boxes, (x, y), (x + w, y + h), (0, 255, 0), 5)
            elif (w>=250):
                self.bounding_boxes.append((x, y, w//2, h))
                self.bounding_boxes.append((x+w//2, y, w//2, h))
                self.image_with_all_bounding_boxes = cv2.rectangle(self.image_with_all_bounding_boxes, (x, y), (x + w//2, y + h), (0, 255, 0), 5)
                self.image_with_all_bounding_boxes = cv2.rectangle(self.image_with_all_bounding_boxes, (x + w//2, y ), (x + w, y + h), (0, 255, 0), 5)
            elif (h>=100):
                self.bounding_boxes.append((x, y, w, h//2))
                self.bounding_boxes.append((x, y+h//2, w, h//2))
                self.image_with_all_bounding_boxes = cv2.rectangle(self.image_with_all_bounding_boxes, (x, y), (x + w , y + h//2), (0, 255, 0), 5)
                self.image_with_all_bounding_boxes = cv2.rectangle(self.image_with_all_bounding_boxes, (x , y+h//2 ), (x + w, y + h), (0, 255, 0), 5)
    ...
    def crop_each_bounding_box_and_ocr(self):
        ...
        for (index,row) in enumerate(self.rows):
            for (rowno,bounding_box) in enumerate(row):
                x, y, w, h = bounding_box
                y = y - 5
                cropped_image = self.original_image[y:y+h, x:x+w]
                image_slice_path = "./ocr_slices/img_" + str(image_number) + ".jpg"
                cv2.imwrite(image_slice_path, cropped_image)
                if index!=0 and rowno==0:
                    results_from_ocr = self.get_result_from_tesseract(image_slice_path,digits=True)
                else:
                    results_from_ocr = self.get_result_from_tesseract(image_slice_path)
                if(results_from_ocr==None):
                    results_from_ocr="yes"
    ...
    def get_result_from_tesseract(self, image_path, digits=False):
        pytesseract.pytesseract.tesseract_cmd = "./cv/tesseract/tesseract"
        if digits:
    ...
    def generate_orderidlist(self):
        ...
        for row in self.table[1:]:
            if len(row)==self.colno:
                orderids.append(int(row[0]))
        return orderids